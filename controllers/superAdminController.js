...
const SickLeave = require("../models/SickLeave"); // Add this line
...
async function getAllMappedDetails(req, res) {
  try {
    ...
    const shifts = await Shift.find({ superAdminId: req.userId })
      .populate("userId", "firstName lastName")
      .populate("workspaceId", "name");
    const timeEntries = await TimeTracker.find({
      userId: { $in: userObjects.map((user) => user?._id) },
    });
    ...
    for (const manager of managers) {
      ...
      const managerVacations = vacations.filter(
        (vacation) =>
          vacation.userId && String(vacation.userId._id) === String(manager._id)
      );
      const managerSickLeaves = sickLeaves.filter(
        (sickLeave) =>
          sickLeave.userId && String(sickLeave.userId._id) === String(manager._id)
      );
      ...
      manager.sickLeaves = managerSickLeaves; // Add this line
    }
    ...
    for (const employee of regularEmployees) {
      ...
      const employeeVacations = vacations.filter(
        (vacation) =>
          vacation.userId &&
          String(vacation.userId._id) === String(employee._id)
      );
      const employeeSickLeaves = sickLeaves.filter(
        (sickLeave) =>
          sickLeave.userId &&
          String(sickLeave.userId._id) === String(employee._id)
      );
      ...
      employee.sickLeaves = employeeSickLeaves; // Add this line
    }
